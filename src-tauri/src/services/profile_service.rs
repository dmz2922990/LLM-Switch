...
pub async fn list(pool: &SqlitePool) -> Result<Vec<Profile>, String> {
    sqlx::query_as::<_, Profile>("SELECT * FROM profiles ORDER BY created_at ASC")
        .fetch_all(pool).await.map_err(|e| format!("Failed to list profiles: {}", e))
}
...
pub async fn rename(pool: &SqlitePool, id: &str, new_name: &str) -> Result<Profile, String> {
    let now = chrono::Utc::now().to_rfc3339();
    let duplicate = sqlx::query_as::<_, Profile>("SELECT * FROM profiles WHERE name = ? AND id != ?")
        .bind(new_name).bind(id).fetch_optional(pool).await
        .map_err(|e| format!("Database error: {}", e))?;
    if duplicate.is_some() {
        return Err("Profile name already exists".to_string());
    }
    sqlx::query_as::<_, Profile>("UPDATE profiles SET name = ?, updated_at = ? WHERE id = ? RETURNING *")
        .bind(new_name).bind(&now).bind(id)
        .fetch_one(pool).await.map_err(|e| format!("Failed to rename profile: {}", e))
}
...
pub async fn duplicate(pool: &SqlitePool, id: &str) -> Result<Profile, String> {
    ...
    loop {
        let exists = sqlx::query_as::<_, Profile>("SELECT * FROM profiles WHERE name = ?")
            .bind(&copy_name).fetch_optional(pool).await
            .map_err(|e| format!("Database error: {}", e))?;
        if exists.is_none() { break; }
        copy_name = format!("{} (副本 {})", source.name, counter);
        counter += 1;
    }
    create(pool, CreateProfile { name: copy_name, settings_json: Some(source.settings_json) }).await
}
...
pub async fn set_active(pool: &SqlitePool, id: &str) -> Result<Profile, String> {
    ...
    sqlx::query("UPDATE profiles SET is_active = 0").execute(pool).await
    ...
    ).bind(&now).bind(id).fetch_one(pool).await.map_err(|e| format!("Failed to activate profile: {}", e))?;
...
pub async fn get_active(pool: &SqlitePool) -> Result<Option<Profile>, String> {
    sqlx::query_as::<_, Profile>("SELECT * FROM profiles WHERE is_active = 1")
        .fetch_optional(pool).await.map_err(|e| format!("Database error: {}", e))
}

pub async fn update_settings(pool: &SqlitePool, id: &str, settings_json: &str) -> Result<Profile, String> {
    let now = chrono::Utc::now().to_rfc3339();
    let profile = sqlx::query_as::<_, Profile>(
        "UPDATE profiles SET settings_json = ?, updated_at = ? WHERE id = ? RETURNING *"
    ).bind(settings_json).bind(&now).bind(id)
        .fetch_one(pool).await.map_err(|e| format!("Failed to update settings: {}", e))?;
    if profile.is_active {
